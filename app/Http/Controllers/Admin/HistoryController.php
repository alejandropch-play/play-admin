...
class HistoryController extends Controller
{
    ...
    public function create(HistoryRequest $request)
    {
        ...
        if ($request->hasFile('responsive_image')) {
            ...
            $store_responsive_image = Storage::disk('gcs')->putFileAs('img/history/responsive/', $request->file('responsive_image'), $responsive_image_name);
            if (!$store_responsive_image) {
                return response()->json(['title' => trans('custom.title.error'), 'message' => trans('custom.errors.image')], 500);
            }
            $history = array_merge($history, ["responsive_image" => $responsive_image_name]);
        }
    ...
    {
        $image = $history->image;
        $responsive_image = $history->responsive_image;

        try {
            $history_delete = $history->delete();
            if ($history_delete) {
                Storage::disk('private')->delete('files/img/history/' . $image);
                Storage::disk('private')->delete('files/img/history/responsive/' . $responsive_image);
            }
    ...
    public function update(HistoryRequest $request, History $history)
    {
        ...
        if ($request->hasFile('responsive_image')) {
            ...
            $store_image = Storage::disk('gcs')->putFileAs('img/history/responsive/', $request->file('responsive_image'), $responsive_image_name);
            if (!$store_image) {
                return response()->json(['title' => trans('custom.title.error'), 'message' => trans('custom.errors.image')], 500);
            }
            $request_history = array_merge($request_history, ["responsive_image" => $responsive_image_name]);
        } else {
            $request_history = array_merge($request_history, ["responsive_image" => $history->responsive_image]);
        }
        if ($request->hasFile('responsive_image') && $history->responsive_image) {
            Storage::disk('gcs')->delete('img/history/responsive/' . $history->responsive_image);
        }
